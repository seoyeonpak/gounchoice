	...
	private void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			case "/order/checkout":
				...
				Object pIdsObj = requestData.get("productIds");

				if (pIdsObj != null && pIdsObj instanceof List) {
					@SuppressWarnings("unchecked")
					List<Object> pIdsRaw = (List<Object>) pIdsObj;
					List<Integer> productIds = new ArrayList<>();
					for (Object obj : pIdsRaw) {
						productIds.add(Integer.parseInt(obj.toString()));
					}

					if (productIds.isEmpty())
						throw new IllegalArgumentException("주문할 상품이 선택되지 않았습니다.");

					resultId = orderService.checkoutSelectedCartItems(userId, addr2, productIds);
