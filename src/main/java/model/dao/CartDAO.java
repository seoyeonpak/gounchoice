...
public class CartDAO {
	...
	public List<CartItem> selectCartItems(Connection conn, int cartId) {
		...
		String sql = "SELECT C.product_id, C.quantity, " + "       P.product_name, P.price, P.product_image "
				+ "FROM CART_ITEM C " + "JOIN PRODUCT P ON C.product_id = P.product_id " + "WHERE C.cart_id = ? "
		...
		try {
			...
			while (rs.next()) {
				list.add(new CartItem(cartId, rs.getInt("product_id"), rs.getInt("quantity"),
						rs.getString("product_name"), rs.getInt("price"), rs.getString("product_image")));
			}
		} catch (SQLException e) {
			e.printStackTrace();
		} finally {
